...
- предлагает только базовую безопасность (с точки зрения прав доступа) на уровне экземпляра. Все СУБД предоставляют подробные списки управления доступом к объектам (или управление ролями).предлагает только базовую безопасность (с точки зрения прав доступа) на уровне экземпляра. Все СУБД предоставляют подробные списки управления доступом к объектам (или управление ролями).
- не масштабируется. Он работает только на одном ядре процессора в однопоточном режиме. Чтобы получить масштабируемость, несколько экземпляров Redis, они должны быть развернуты и запущены
